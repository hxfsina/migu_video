...
async function incrementalSyncNonMemberVideos() {
  ...
  for (const cid of allCategories) {
    ...
    try {
      ...
      console.log(`数据库中已有 ${Object.keys(existingVideos).length} 个 ${categoryName} 非会员视频`);
      ...
      while (hasMoreData) {
        ...
        // 🔥 关键修改：分别获取不同付费类型的视频，然后合并
        for (const payType of nonMemberPayTypes) {
          try {
            const videos = await fetchMiguCategory(cid, currentPage, 20, { payType });
